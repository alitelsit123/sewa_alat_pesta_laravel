...
class KeranjangController extends Controller
{
    public function showCartPage()
    {
        ...
        $keranjang_total_kuantitas = array_sum(
            array_column($keranjangs_pivot, 'kuantitas')
        );

        $keranjang_total_price = array_sum(
            array_column($keranjangs_array, 'total')
        );
    ...
    public function addToCart(Request $request)
    {
        ...
        $produk = Produk::withOrdered()
            ->whereId_produk($request->all()['produk_id'])
            ->first();
        $max_produk =
            (int) $produk->stok - (int) $produk->ordered_sum_kuantitas;
        ...
        if ($user) {
            ...
            return back()->with([
                'msg_success' => 'Produk Di tambahkan ke Keranjang',
            ]);
        } else {
            $keranjang = new Cart([
                'kuantitas' => $validated_input['kuantitas'],
        ...
        if ($user) {
            ...
            foreach ($validated_input as $index => $item) {
                ...
                $keranjang = $user
                    ->carts()
                    ->withOrdered()
                    ->where('produk.id_produk', $r_id[1])
                    ->first();
                if (
                    $keranjang &&
                    $keranjang->stok - $keranjang->ordered_sum_kuantitas >=
                        $item
                ) {
                    ...
                    return back()->with([
                        'msg_error' =>
                            ...
                            ' melebihi batas stok. mohon Update!',
                    ]);
                }
            }
            ...
            foreach ($validated_input as $index => $item) {
                ...
                if (
                    $current_cart &&
                    $current_cart->stok -
                        $current_cart->ordered_sum_kuantitas >=
                        $item
                ) {
                    ...
                    return back()->with([
                        'msg_error' => 'Kuantitas melebihi batas',
                    ]);
                }
    ...
    public function deleteCart($id)
    {
        ...
        if ($user) {
            ...
            return back()->with([
                'msg_success' => 'Produk Di hapus dari Keranjang',
            ]);
        } else {
            $old_carts = collect(session('cart'));
