...
class OrderController extends Controller
{
    ...
    public function checkData(Request $request)
    {
        ...
        $validators = [
            ...
            'alamat' =>
                $user->profile->addresses()->count() == 0 ? null : 'filled',
        ];
        foreach ($validators as $item) {
            if (!$item) {
                return back()->with([
                    'msg_error' =>
                        'Mohon lengkapi (nama, nomor hp, alamat) biodata Anda!',
                ]);
            }
        }
    ...
    public function changeBookDuration(Request $request)
    {
        ...
        $cartWithStat = $user->cartWithStat(
            array_column($old_cart_maintain['maintains'], 'id_produk')
        );
    ...
    public function checkoutView()
    {
        ...
        $data = [
            ...
            'stat' => auth()
                ->user()
                ->cartWithStat(
                    array_column(session('cart')['maintains'], 'id_produk')
                )['stat'],
        ];
    ...
    public function processPayment(Request $request)
    {
        ...
        $check_order = $user
            ->order()
            ->where('tanggal_mulai', $validated_input['tanggal_mulai'])
            ->whereIn('status', [1, 2])
            ->get();
    ...
    public function makePayment(Request $request)
    {
        if (
            !session()->has('cart') ||
            !session()->has('additional_data_order')
        ) {
            return redirect('/cart');
        }
        ...
        $filled = DB::transaction(function () {
            ...
            $pay_total =
                $additional_data_order['tipe_pembayaran'] == 1
                    ? (40 * (int) $stats['total_bayar']) / 100
                    : (int) $stats['total_bayar'];
            ...
            array_map(function ($item) use ($order, $user) {
                $order->details()->create([
                    ...
                    'total_harga' =>
                        $item['pivot']['kuantitas'] * $item['harga'],
                ]);
    ...
    public function paymentNotification(Request $request)
    {
        ...
        if ($payment) {
            if ($status == 'settlement' && $status_code == '200') {
                ...
                if (!$sewa) {
                    ...
                    foreach ($user_admins as $row) {
                        $notification_admin = new NewOrderCreatedNotification(
                            $order
                        );
                ...
                foreach ($user_admins as $row) {
                    $notification_admin = new UserPaymentSuccessNotification(
                        $payment
                    );
                    $row->notify($notification_admin);
                }
            ...
            } elseif ($status == 'pending' && $status_code == '201') {
            } elseif ($status == 'cancel' && $status_code == '202') {
                $order->status = 5;
                $order->save();
                $order->hapus();
            } else {
