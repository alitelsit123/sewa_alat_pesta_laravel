...
@auth
@if(auth()->user()->sesiChat)
var chat_history = JSON.parse('{!! auth()->user()->sesiChat()->with('chats')->first()->toJson() !!}');
    ...
    $.post('{{ route("search-online-cs") }}', {
        ...
        if(data.cs) {
            if(data.status == 1) {
                ...
                insertCenterChat(data.msg, element_id_2);
                setTimeout(function() {
